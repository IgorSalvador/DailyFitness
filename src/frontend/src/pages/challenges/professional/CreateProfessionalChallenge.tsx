...
const CreateProfessionalChallenge = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await createManagedChallenge({
        name: name.trim(),
      ...
      if (!res.success) {
        setErrors([res.message || "Falha ao criar desafio."]);
        return;
      }
      ...
      navigate("/desafios/profissional");
    } catch (err) {
      setErrors([err instanceof Error ? err.message : "Erro inesperado ao criar desafio."]);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="glass-card rounded-2xl p-8 space-y-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Preencha os dados para criar um novo desafio e compartilhá-lo com a plataforma.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              >
                <option value="" disabled>Selecione o tipo do desafio</option>
                {(Object.values(EChallengeType).filter((v) => typeof v === "number") as EChallengeType[]).map((t) => (
                  <option key={t} value={t}>{ChallengeTypeLabel[t]}</option>
                ))}
              </select>
              <p className="text-xs text-muted-foreground mt-1">
                Define a periodicidade dos registros de progresso. Não poderá ser alterado após usuários ingressarem.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              >
                {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Criando...</> : "Criar Desafio"}
              </button>
              <Link
                to="/desafios/profissional"
...
export default CreateProfessionalChallenge;
